...
import { setImageFallback, getPositionName, extractArray, formatNumberWithDots } from '../../utils/helpers';
...
const Lineup = ({ teamId: propTeamId }) => {
  ...
  const marketPlayers = useMemo(() => extractArray(marketData), [marketData]);
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {lineupData && formation ? (
        <div className="space-y-6">
          ...
          <div className="rounded-lg p-3 sm:p-6 pb-24 sm:pb-12 min-h-[500px] sm:min-h-[600px] relative overflow-hidden" style={{background: `linear-gradient(to bottom, #0A6522, #0A6522)`}}>
            {/* Center line at top */}
                    ...
                    <div className="flex justify-center gap-2 sm:gap-4 w-full max-w-md">
                      {row.positions.map((positionId, playerIndex) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-14 h-14 sm:w-20 sm:h-20 m-2 sm:m-2 ${isGoalkeeperRow ? '-mt-12 sm:-mt-16' : '-mt-6 sm:-mt-10'} bg-white/90 dark:bg-gray-800/90 rounded-full border-2 border-gray-300 dark:border-gray-600 flex items-center justify-center shadow-lg hover:scale-110 transition-transform relative cursor-pointer`}
                            ...
                            {player && (() => {
                              ...
                              return (
                                <div className="absolute top-full mt-1 text-center w-24 left-1/2 transform -translate-x-1/2 pointer-events-none">
                                  {/* Team Badge */}
                                  {(() => {
                                    const pMaster = player.playerMaster || player;
                                    const badgeUrl = pMaster.team?.badgeColor || pMaster.team?.badge || pMaster.team?.logo;
                                    if (!badgeUrl) return null;
                                    return (
                                      <img 
                                        src={badgeUrl} 
                                        alt={pMaster.team?.name || ''} 
                                        className="absolute -top-14 right-2 w-5 h-5 object-contain drop-shadow-md bg-white/80 dark:bg-gray-800/80 rounded-full p-0.5 pointer-events-none" 
                                      />
                                    );
                                  })()}

                                  <div className="text-[10px] sm:text-xs font-bold text-white drop-shadow-md truncate bg-black/40 px-1 rounded">
                                    {playerName}
                                  </div>
                                  {trendData && (
                                ...
                                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 px-3 py-2 rounded-lg text-xs opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-xl pointer-events-none border border-gray-700 dark:border-gray-300">
                                  ...
                                  <div className="flex justify-between gap-4 mb-1 items-center">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span>{getPositionName(player.positionId)}</span>
                                    </div>
                                    {(() => {
                                      const weekPoints = getWeekPoints(player, selectedWeek);
                                      return weekPoints !== undefined && (
                                        <span className="font-semibold text-blue-400 dark:text-blue-600">{weekPoints} pts</span>
                                      );
                                    })()}
                                  </div>
