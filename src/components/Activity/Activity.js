...
import { RefreshCw, ChevronDown, Activity as ActivityIcon, FileDown } from 'lucide-react';
...
import BudgetReport from '../Teams/BudgetReport';
...
const Activity = () => {
  ...
  const userTeamId = useMemo(() => {
    const entry = standingsData.find((team) => {
  ...
  // El informe recorre el histórico COMPLETO, no las 5 páginas que pinta esta
  // pantalla: para cuadrar saldos hacen falta todos los movimientos.
  const { ledger, selfCheck, ownManagerId } = useTeamBudgets(leagueId, ranking, userTeamId);

  const managersCache = useMemo(() => buildManagersCache(ranking), [ranking]);
  const getUserNameById = useCallback(
    (id) => managersCache.get(String(id))?.managerName || null,
    [managersCache],
  );
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Descargar PDF con todos los movimientos"
            title={ledger ? 'Genera un PDF con el histórico completo' : 'Cargando histórico completo…'}
          >
            <FileDown className="w-4 h-4" aria-hidden="true" />
            {ledger ? 'Descargar PDF' : 'Preparando…'}
          </button>
          <button
            type="button"
      ...
      {/* El informe se monta en <body>: la hoja de impresión oculta todo lo que
          no sea su contenedor, y eso exige que sea hermano de #root. */}
      {ledger && createPortal(
        <div className="budget-report-host">
          <BudgetReport
            ledger={ledger}
            selfCheck={selfCheck}
            ownManagerId={ownManagerId}
            ownName={ownManagerId ? getUserNameById(ownManagerId) : null}
