...
const main = async () => {
    ...
    // Nombres de manager por equipo: los necesita clause-alerter para decir de
    // quién es el jugador, y allí no se vuelve a pedir la clasificación.
    const managers = {};
    for (const e of clasificacion) {
        const id = e.id || e.team?.id;
        const nombre = e.team?.manager?.managerName || e.manager || e.name;
        if (id && nombre) managers[String(id)] = nombre;
    }
    ...
    await fs.writeFile(SALIDA, `${JSON.stringify({
        ...
        costes,
        historial,
        // Lo mantiene clause-alerter; aquí solo se conserva entre ejecuciones.
        avisados: estado?.avisados || {},
    }, null, 2)}\n`, 'utf8');
