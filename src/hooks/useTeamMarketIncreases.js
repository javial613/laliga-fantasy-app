...
const useTeamMarketIncreases = (standings, leagueId, trendsReady) => {
    ...
    // La suma por equipo se deriva de la caché de tendencias, que vive en un
    // singleton fuera de React Query. Sin el sello temporal del scrape en la
    // key, un refresco de tendencias no invalidaría estos totales y la
    // clasificación seguiría sumando las variaciones del scrape anterior.
    const trendsStamp = marketTrendsService.lastMarketScrape
        ? new Date(marketTrendsService.lastMarketScrape).getTime()
        : 0;

    const { data } = useQuery({
        queryKey: ['teamMarketIncreases', leagueId, standingsCount, trendsStamp],
        queryFn: async () => {
