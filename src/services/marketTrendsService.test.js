...
describe('marketTrendsService.initialize (revalidación de la caché)', () => {
    afterEach(() => {
        marketTrendsService.initialized = false;
        marketTrendsService._initPromise = null;
        marketTrendsService.marketValuesCache = new Map();
        marketTrendsService.lastMarketScrape = null;
        jest.restoreAllMocks();
    });

    test('no vuelve a scrapear si ya está inicializado y la caché es reciente', async () => {
        marketTrendsService.initialized = true;
        marketTrendsService.lastMarketScrape = new Date();
        marketTrendsService.marketValuesCache = new Map([['x', cacheEntry()]]);
        const fetchSpy = jest.spyOn(marketTrendsService, 'fetchMarketValues');

        const result = await marketTrendsService.initialize();

        expect(fetchSpy).not.toHaveBeenCalled();
        expect(result.source).toBe('already_initialized');
    });

    test('vuelve a scrapear cuando la caché caduca aunque ya esté inicializado', async () => {
        marketTrendsService.initialized = true;
        marketTrendsService.lastMarketScrape = new Date(Date.now() - 7 * 60 * 60 * 1000);
        marketTrendsService.marketValuesCache = new Map([['x', cacheEntry()]]);
        jest.spyOn(marketTrendsService, 'loadCachedData').mockImplementation(() => {});
        const fetchSpy = jest
            .spyOn(marketTrendsService, 'fetchMarketValues')
            .mockResolvedValue({ success: true, playersCount: 1, source: 'real' });
    ...
    test('la caché caduca a las 6 horas, no a las 24', () => {
        marketTrendsService.lastMarketScrape = new Date(Date.now() - 5 * 60 * 60 * 1000);
        expect(marketTrendsService.isCacheStale()).toBe(false);

        marketTrendsService.lastMarketScrape = new Date(Date.now() - 7 * 60 * 60 * 1000);
        expect(marketTrendsService.isCacheStale()).toBe(true);
    });
});
