...
/**
 * El histórico completo alimenta los saldos estimados de Equipos y el informe
 * en PDF. Se invalida en vez de refetchear porque recorrerlo entero es caro
 * (decenas de páginas con pausas): así se marca como caducado y se recarga solo
 * cuando alguien abre una pantalla que lo necesita, no en cada operación.
 *
 * Y a diferencia de `refetchAll`, esto alcanza a las queries no montadas, que
 * es justo el caso: vendes desde Mercado y el saldo lo miras luego en Equipos.
 */
const invalidateBudgetHistory = (queryClient, leagueId) =>
  queryClient.invalidateQueries({ queryKey: ['leagueActivityFull', leagueId] });
...
export const invalidateMarketData = async (queryClient, leagueId) => {
  await invalidateBudgetHistory(queryClient, leagueId);
...
export const invalidateTeamMoney = async (queryClient, teamId) => {
  // Sin `type: 'active'`: el saldo se consulta desde varias pantallas y hay que
  // caducarlo también en las que no están montadas.
  await queryClient.invalidateQueries({ queryKey: ['teamMoney', teamId] });
