...
import useStarterProbabilities from '../../hooks/useStarterProbabilities';
...
const Market = () => {
  ...
  // --- Rival de la próxima jornada, compartido por todas las tarjetas ---
  const nextFixtures = useNextFixtures();

  // --- Probabilidad de titularidad, solo de los equipos presentes en el mercado ---
  const equiposEnMercado = useMemo(() => {
    const ids = new Set();
    for (const item of extractPlayers(marketData)) {
      const id = item?.playerMaster?.team?.id ?? item?.playerMaster?.teamId;
      if (id != null) ids.add(String(id));
    }
    return [...ids];
  }, [marketData]);
  const probabilidadesTitularidad = useStarterProbabilities(equiposEnMercado);
  ...
  const filteredPlayers = useMemo(() => {
    return playersArray
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          case 'trend': {
            const aTrend = marketTrendsService.resolveTrendForPlayer(a.playerMaster)?.diferencia1 || 0;
            const bTrend = marketTrendsService.resolveTrendForPlayer(b.playerMaster)?.diferencia1 || 0;
    ...
    // trendsReady entra en las deps para reordenar cuando la caché de
    // tendencias termina de cargar (el orden por defecto es 'trend'). No se
    // lee dentro del memo porque la caché vive en el singleton del servicio.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [playersArray, searchTerm, positionFilter, priceFilter, ownerFilter, sortBy, trendsReady]);
            ...
            {filteredPlayers.map((item, index) => (
              ...
              >
                <PlayerListItem
                  ...
                  probabilidadesTitularidad={probabilidadesTitularidad}
                />
              </motion.div>
            ))}
          </div>
