...
export const normalizePlayer = (p, teamsMap) => {
  ...
  // La foto local, si existe, manda sobre la de la API (ver playerImageOverrides).
  const override = getPlayerImageOverride(p);
  const imageUrl = override || p.image || (p.images && p.images.transparent && p.images.transparent['256x256']) || null;
  return {
    ...p,
    ...
    images: imageUrl
      ? {
          ...(p.images || {}),
          transparent: { ...((p.images && p.images.transparent) || {}), '256x256': imageUrl },
          player: (p.images && p.images.player) || imageUrl,
        }
      : p.images,
  };
};
...
/**
 * Normaliza el `playerMaster` que cuelga de una entrada de plantilla, mercado
 * o alineación. La API v6 devuelve estos jugadores igual de pelados que los de
 * /v6/players — con `teamId` pero sin objeto `team` —, así que sin esto las
 * tarjetas del mercado y los jugadores del campo se quedan sin nombre de
 * equipo ni escudo.
 */
const normalizePlayerContainer = (entry, teamsMap) => {
  if (!entry || typeof entry !== 'object' || !entry.playerMaster) return entry;
  return { ...entry, playerMaster: normalizePlayer(entry.playerMaster, teamsMap) };
};

const normalizeContainerList = (list, teamsMap) =>
  list.map((entry) => normalizePlayerContainer(entry, teamsMap));

/**
 * Mercado: `data` es el array de anuncios, cada uno con su `playerMaster`.
 */
export const createAdaptMarketResponse = (loadTeamsMaster) => async (response) => {
  ...
  if (!list) return response;
  const teamsMap = await loadTeamsMaster();
  const normalized = normalizeContainerList(list, teamsMap);
  return {
    ...response,
...
/**
 * Datos de equipo: la plantilla vive en `data.players`.
 */
export const createAdaptTeamDataResponse = (loadTeamsMaster) => async (response) => {
  const payload = response?.data?.data || response?.data;
  if (!payload || !Array.isArray(payload.players)) return response;
  const teamsMap = await loadTeamsMaster();
  const players = normalizeContainerList(payload.players, teamsMap);
  const patched = { ...payload, players };
  return {
    ...response,
    data: response.data?.data ? { ...response.data, data: patched } : patched,
  };
};

/**
 * Alineación: cada línea de `data.formation` es un array de jugadores.
 */
export const createAdaptLineupResponse = (loadTeamsMaster) => async (response) => {
  const payload = response?.data?.data || response?.data;
  const formation = payload?.formation;
  if (!formation || typeof formation !== 'object') return response;

  const teamsMap = await loadTeamsMaster();
  const patchedFormation = { ...formation };
  for (const line of ['goalkeeper', 'defender', 'midfield', 'striker']) {
    if (Array.isArray(formation[line])) {
      patchedFormation[line] = normalizeContainerList(formation[line], teamsMap);
    }
  }
  const patched = { ...payload, formation: patchedFormation };
  return {
    ...response,
    data: response.data?.data ? { ...response.data, data: patched } : patched,
  };
