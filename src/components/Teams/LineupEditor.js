...
import {
  ...
  AlertCircle, User, X, Search, Grid3x3, Loader, Clock, Shield, ShoppingCart, DollarSign
} from 'lucide-react';
import { fantasyAPI } from '../../services/api';
import { useAuthStore } from '../../stores/authStore';
import LoadingSpinner from '../Common/LoadingSpinner';
import toast from 'react-hot-toast';
import { extractArray, formatNumberWithDots, getPositionName } from '../../utils/helpers';
...
import NextFixture from '../Common/NextFixture';

/**
 * URL del escudo del equipo de un jugador.
 *
 * El endpoint de alineación (`/teams/:id/lineup`) devuelve los jugadores con
 * datos recortados, así que leer `team.badgeColor` a secas deja sin escudo a
 * los jugadores del campo. La plantilla de `teamData` sí trae el playerMaster
 * completo, y se usa como segunda fuente indexada por id de jugador.
 *
 * No hay tercer fallback por id de equipo: la URL
 * `assets-fantasy.llt-services.com/teams/:id/badge.png` que usan Ver
 * Alineaciones y Equipos de la Liga responde 404 para todos los equipos
 * probados, así que solo serviría para pintar imágenes rotas.
 */
const resolveTeamBadge = (pMaster, playerMasterById) => {
  const fromTeam = (team) => team?.badgeColor || team?.badge || team?.logo;

  const direct = fromTeam(pMaster?.team);
  if (direct) return direct;

  return fromTeam(playerMasterById?.get(pMaster?.id)?.team) || null;
};
...
const LineupEditor = () => {
  ...
  // Índice de la plantilla por id de jugador: la alineación viene con datos
  // recortados, así que sirve de fuente completa para escudo y equipo.
  const playerMasterById = React.useMemo(() => {
    const map = new Map();
    for (const playerTeam of teamPlayers) {
      const pMaster = playerTeam?.playerMaster;
      if (pMaster?.id) map.set(pMaster.id, pMaster);
    }
    return map;
  }, [teamPlayers]);
  ...
  const marketPlayers = React.useMemo(() => extractArray(marketData), [marketData]);
  ...
  // --- Rival de la próxima jornada alineable, compartido por la plantilla ---
  ...
  const [, setPendingMarketOperations] = useState(new Set());
  
  const getPlayerMarketData = useCallback((playerId) => {
      return marketPlayers.find(m => m.playerMaster?.id === playerId || m.playerTeam?.playerTeamId === playerId || m.player?.id === playerId);
  }, [marketPlayers]);

  const handleSellClick = (player) => {
    const pMaster = player.playerMaster || player;
    const isMarket = getPlayerMarketData(pMaster.id);
    setSelectedMarketPlayer({ player: pMaster, playerTeam: player });
    if (isMarket) {
      withdrawFlow.open();
    } else {
      marketListFlow.open();
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {formationReq && (
        <div className="card p-4 sm:p-6">
          <div className="mb-3 sm:mb-4">
            ...
            {/* La jornada se indica aquí una sola vez: repetirla bajo los once
                jugadores sería ruido, y hace falta saberla porque el rival que
                se muestra salta a la jornada siguiente en cuanto arranca ésta. */}
            {nextFixtures.weekNumber != null && (
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                Rivales de la jornada {nextFixtures.weekNumber}
              </p>
            )}
          </div>

          {/* Football Field */}
          <div className="relative bg-gradient-to-b from-green-600 to-green-700 rounded-xl p-3 sm:p-8 min-h-[460px] sm:min-h-[700px]">
            {/* Field markings */}
            ...
            {/* Players */}
            <div className="relative h-full flex flex-col pt-3 sm:pt-6 pb-12 sm:pb-12">
              {/* Strikers - Muy adelantados */}
              {/* Los márgenes entre filas deben superar la altura de la
                  etiqueta del jugador (nombre + tendencia + próximo rival:
                  48px en escritorio, 44px en móvil), porque va posicionada
                  en absoluto y no ocupa espacio en el flujo. El contenedor
                  interno usa h-full sobre un padre que solo tiene
                  min-height, así que ese h-full no resuelve y flex-1 no
                  reparte nada: la separación real es solo la de estos
                  márgenes. Medido: deja 12px de aire en móvil y 24px en
                  escritorio. */}
              <div className="mb-16 sm:mb-20">
                <PositionRow
                  ...
                  onSell={(player) => handleSellClick(player)}
                  marketPlayers={marketPlayers}
                  playerMasterById={playerMasterById}
      ...
      <MarketListFlow
        ...
        onReset={() => setSelectedMarketPlayer(null)}
      />
    </div>
  );
};
...
// Helpers for rows
const PositionRow = ({ players, count, positionType, selectedPosition, onSlotClick, onPlayerRemove, onSell, marketPlayers, playerMasterById, nextFixtures }) => {
  return (
    // La separación debe superar el ancho de la etiqueta del nombre (w-28,
    // 112px) para que las de dos jugadores contiguos no lleguen a tocarse:
    // 80px de hueco + 40px de gap = 120px entre centros.
    <div className="flex justify-center gap-3 sm:gap-10 w-full max-w-2xl px-2 mx-auto">
...
const PlayerSlot = ({ player, onClick, onRemove, isSelected, onSell, marketPlayers = [], playerMasterById, nextFixtures }) => {
  return (
    <div className="relative group">
      ...
      >
        {player ? (
          <div className="text-center relative w-full h-full flex flex-col items-center">
            {player.playerMaster?.images?.transparent?.['256x256'] ? (
              <img
                ...
                className="w-14 h-14 sm:w-20 sm:h-20 object-contain rounded-full border-2 border-white/50 shadow-sm"
              />
            ) : (
              <div className="w-14 h-14 sm:w-20 sm:h-20 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center border-2 border-white/50">
            ...
            {/* Nombre y Tendencia bajo el jugador */}
            {(() => {
              const pMaster = player.playerMaster || player;
              const playerName = pMaster.nickname || pMaster.name || 'Jugador';
              const trendData = marketTrendsService.resolveTrendForPlayer(pMaster);
              
              const badgeUrl = resolveTeamBadge(pMaster, playerMasterById);

              return (
                <div className="absolute top-full mt-1 w-28 left-1/2 transform -translate-x-1/2 pointer-events-none flex flex-col items-center">
                  {/* La píldora se ajusta al contenido (no ancho fijo) para que
                      las etiquetas de dos jugadores contiguos no se solapen. */}
                  <div className="max-w-full text-[10px] sm:text-xs font-bold text-white drop-shadow-md bg-black/40 px-1 rounded flex items-center justify-center gap-1">
                    {badgeUrl && (
                      <img
                        src={badgeUrl}
                        alt={pMaster.team?.name || ''}
                  ...
                  {trendData && (
                    <div className={`text-[9px] sm:text-[10px] font-bold drop-shadow-md flex items-center justify-center ${
                      trendData.isPositive ? 'text-green-400' :
                      trendData.isNegative ? 'text-red-400' : 'text-gray-200'
                    }`}>
                      {trendData.isPositive ? '▲ ' : trendData.isNegative ? '▼ ' : ''}
                      {trendData.cambioTexto}
                    </div>
                  )}
                  <NextFixture
                    teamId={pMaster.team?.id ?? pMaster.teamId}
                    nextFixtures={nextFixtures}
                    variant="pitch"
                    className="mt-0.5"
                  />
            ...
            {/* El escudo del equipo se muestra junto al nombre, bajo el jugador */}

            {/* Remove button */}
            <button
              className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center text-white shadow-md transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                onRemove();
              }}
            ...
            {/* Market action button overlay */}
            {(() => {
              const pMaster = player.playerMaster || player;
              const marketInfo = marketPlayers?.find(m => m.playerMaster?.id === pMaster.id);
              return (
                <button
                  className={`absolute -top-1 -left-1 w-5 h-5 ${marketInfo ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-500 hover:bg-green-600'} rounded-full flex items-center justify-center text-white shadow-md transition-colors z-10`}
                  onClick={(e) => {
                    e.stopPropagation();
                    onSell && onSell(player);
                  }}
                  title={marketInfo ? 'Retirar del mercado' : 'Vender al mercado'}
                >
                  {marketInfo ? <X className="w-3 h-3" /> : <DollarSign className="w-3 h-3" />}
                </button>
              );
            })()}
            ...
            {(() => {
              const pMaster = player.playerMaster || player;
              const ownerInfo = playerOwnershipService.getPlayerOwner(pMaster.id);
              const marketInfo = marketPlayers?.find(m => m.playerMaster?.id === pMaster.id);
              
              let marketTimeLeft = null;
              if (marketInfo && marketInfo.expirationDate) {
                const expDate = new Date(marketInfo.expirationDate);
                const hoursLeft = Math.max(0, Math.ceil((expDate - new Date()) / (1000 * 60 * 60)));
                marketTimeLeft = `${hoursLeft}h`;
              }
              
              return (
                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 px-3 py-2 rounded-lg text-xs opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-xl pointer-events-none border border-gray-700 dark:border-gray-300">
                  <div className="font-bold text-sm border-b border-gray-700 dark:border-gray-300 pb-1 mb-1">
                    {pMaster.nickname || pMaster.name || 'Jugador'}
                  </div>
                  <div className="flex justify-between gap-4 mb-1 items-center">
                    <div className="flex items-center gap-1.5">
                      {pMaster.team?.badgeColor && (
                        <img src={pMaster.team.badgeColor} alt={pMaster.team.name || ''} className="w-4 h-4 object-contain" />
                      )}
                      <span>{getPositionName(pMaster.positionId)}</span>
                    </div>
                    {player.points !== undefined && (
                      <span className="font-semibold text-blue-400 dark:text-blue-600">{player.points} pts</span>
                    )}
                  </div>
                  
                  {/* Cláusula */}
                  {ownerInfo?.buyoutClause && (
                    <div className="flex items-center gap-1 mt-1 pt-1 border-t border-gray-700 dark:border-gray-300">
                      <Shield className="w-3 h-3 text-blue-400" />
                      <span>Cláusula: <span className="font-bold">{formatNumberWithDots(ownerInfo.buyoutClause)}€</span></span>
                    </div>
                  )}
                  
                  {ownerInfo?.buyoutClauseLockedEndTime && (
                    <div className="flex items-center gap-1 text-[10px] text-gray-300 dark:text-gray-600 mt-0.5">
                      <Clock className="w-3 h-3" />
                      <span>
                        {new Date(ownerInfo.buyoutClauseLockedEndTime) > new Date() 
                          ? `Libre en ${getClauseTimeRemaining(ownerInfo.buyoutClauseLockedEndTime)}`
                          : 'Disponible'}
                      </span>
                    </div>
                  )}
                  ...
                  {/* Mercado */}
                  {marketInfo && (
                    <div className="flex items-center gap-1 mt-1 text-red-400 dark:text-red-600 font-semibold">
                      <ShoppingCart className="w-3 h-3" />
                      <span>En mercado ({marketTimeLeft})</span>
                    </div>
                  )}
                </div>
              );
...
const PositionPlayersList = ({ title, players, lineup, color, onSell, marketPlayers = [], nextFixtures }) => {
      ...
      <div className="space-y-2 max-h-96 overflow-y-auto">
        {players.length === 0 ? (
          ...
          players.map((playerTeam) => {
            ...
            const marketInfo = marketPlayers.find(m => m.playerMaster?.id === player.id);
            ...
            return (
              ...
              >
                {/* Botón de mercado para el banquillo */}
                <button
                  className={`absolute -top-2 -left-2 w-6 h-6 ${marketInfo ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-500 hover:bg-green-600'} rounded-full flex items-center justify-center text-white shadow-md transition-opacity opacity-0 group-hover:opacity-100 z-10`}
                  onClick={(e) => {
                    e.stopPropagation();
                    onSell && onSell(playerTeam);
                  }}
                  title={marketInfo ? 'Retirar del mercado' : 'Vender al mercado'}
                >
                  {marketInfo ? <X className="w-3 h-3" /> : <DollarSign className="w-3 h-3" />}
                ...
                <div className="flex items-start gap-3">
                  {/* Player Image */}
                  <div className="w-10 h-10 rounded-full bg-white dark:bg-gray-700 flex items-center justify-center overflow-hidden flex-shrink-0 border border-gray-200 dark:border-gray-600 relative">
                    {player.images?.transparent?.['256x256'] ? (
                      <img
                        src={player.images.transparent['256x256']}
                        alt={player.nickname || player.name}
                  ...
                  {/* Player Info */}
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 dark:text-white text-sm truncate flex items-center gap-1.5">
                      {player.nickname || player.name}
                      {player.team?.badgeColor && (
                        <img src={player.team.badgeColor} alt={player.team.name || ''} className="w-3 h-3 object-contain inline-block" />
                      )}
                    </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <span className={`text-[10px] font-semibold flex items-center ${
                            trendData.isPositive ? 'text-green-500' :
                            trendData.isNegative ? 'text-red-500' : 'text-gray-400'
                          }`}>
                            {trendData.isPositive ? '▲ ' : trendData.isNegative ? '▼ ' : ''}
                            {trendData.cambioTexto}
                          </span>
