  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (canUseElectronOAuth) {
        ...
        throw new Error('El inicio de sesión automático solo funciona en la app de escritorio. En el navegador, usa el método manual (Pegar Token) de abajo.');
      }
    } catch (err) {
      oauthLog('handleGoogleLogin error:', err.message);
      setError(err.message || 'Error al iniciar sesión con la cuenta de LaLiga');
      setShowManualGuide(true);
      setLoading(false);
    }
  };
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Iniciar sesión con mi cuenta LaLiga</span>
                  </>
                )}
              </motion.button>

              <p className="text-center text-sm text-gray-500">
                Se abrirá la ventana de inicio de sesión de La Liga, donde puedes entrar con Apple, Google o tu correo. Al terminar, volverás automáticamente.
              </p>
